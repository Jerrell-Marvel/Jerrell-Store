...
function ShowProducts({
  ...
  const [data, setData] = useState<ApiResponse<ShowProductsType> | undefined>(
    undefined
  );
        ...
        {loading ? (
          <ul className="grid w-full grid-cols-card-grid gap-6 py-10">
            {[...Array(10)].map((element, index) => {
              return (
                <li
                  className="flex h-96 flex-col gap-4 rounded-xl bg-white p-4 transition-all duration-300"
                  key={index}
                >
            ...
            {data?.data?.map((d) => {
              return (
                <Link to={`/products/itemCategory/${d.id}`} key={d.id}>
                  <li className="flex h-full flex-col gap-4 rounded-xl bg-white p-4 transition-transform duration-300 hover:scale-105 lg:hover:scale-110">
